...
export class SourceController {
  ...
  @ApiOperation({
    ...
    description:
      'Create a source from formData. If a file is included: uploads it to S3 first, stores the URL in DB (File table), then creates the Source linked to that file. If no file: creates source with metadata only (optional fileId for existing vault file). Only CONTRIBUTOR or OWNER can create.',
  })
  ...
  @ApiBody({
    ...
    schema: {
      ...
      properties: {
        ...
        file: { type: 'string', format: 'binary', description: 'File to upload (PDF, DOCX, image, etc.) – uploaded to S3, then URL stored in DB' },
      },
    },
  })
